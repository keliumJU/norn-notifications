...
function UserProfile() {
	...
	useEffect(() => {
		...
		if (user_id) {
			const urlUserName = `${BASE_URL}api/users/${user_id}`
			const urlUserImg = `${BASE_URL}api/user_profile/${user_id}`
	...
	const fetchUserProfileCreate = async (user_id) => {
		const url = `${BASE_URL}api/user_profile/create`
		const data = new FormData()
		data.append('user_id', user_id)
		data.append('full_name', userProfile.full_name)
		data.append('phone', userProfile.phone)
		data.append('mobile', userProfile.mobile)
		data.append('address', userProfile.address)
		data.append('web_site', userProfile.web_site)
		data.append('github', userProfile.github)
		data.append('twitter', userProfile.twitter)
		data.append('facebook', userProfile.facebook)
	...
	const fetchUserProfileUpdate = async (user_id) => {
		const url = `${BASE_URL}api/user_profile/${user_id}/edit`
		const data = new FormData()
		data.append('full_name', userProfile.full_name)
		data.append('phone', userProfile.phone)
		data.append('mobile', userProfile.mobile)
		...
		data.append('facebook', userProfile.facebook)
		data.append('img', file)
		data.append('img_no_update', userProfile.img)
		try {
			const response = await fetch(url, {
				method: 'PUT',
				body: data,
	...
	const handleUpdateUserProfile = () => {
		...
		if (userProfile.err) {
			if (user_id) {
				fetchUserProfileCreate(user_id)
			}
		} else {
			if (user_id) {
									...
									<div className="d-flex flex-column align-items-center text-center">
										{
											editMode ?
												<>
													...
													<input className="form-control" type="file" name="img" id="img" onChange={handleInputChange} />
												</>
												:
												<img src={userProfile.img ? userProfile.img : noimg} alt="Admin" className="rounded-circle" width="150" />
										}
										...
										<div className="mt-3">
											...
											<p className="mb-1">{userAccount.roles}</p>
									...
									<div className="row mb-3">
										{
											editMode ?
												...
												<>
													<div className="col-sm-12">
														<button className="btn btn-info" onClick={handleEditMode}>Editar</button>
													</div>
												</>
										}
